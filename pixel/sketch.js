...
function draw() {
  ...
  mouseXPos = constrain(mouseXPos, 50, width);
  mouseYPos = constrain(mouseYPos, 50, height);
  ...
  for (let y = 0; y < height; y += 50) {
    for (let x = 0; x < width; x += 50) {
      ...
      let containerWidth = map(mouseX, 0, width, 50, 200);
      let containerHeight = map(mouseY, 0, width, 50, 200);
