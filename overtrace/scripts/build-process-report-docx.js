...
function parseArgs(argv) {
  const out = {
    ...
    hubThreshold: 4,
    maxWeeks: 200,
    title: "Overtrace Process Analysis Report",
  };
  const args = [...argv];
  while (args.length) {
    const a = args.shift();
...
function writeDocx(outPath, documentXml, title, mediaParts = []) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "overtrace-docx-"));
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties"
  ...
  <dc:creator>Overtrace</dc:creator>
  <cp:lastModifiedBy>Overtrace</cp:lastModifiedBy>
...
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"
  ...
  <Application>Overtrace</Application>
</Properties>`;
...
function main() {
  ...
  const outDocx = opts.output || path.join(cwd, "overtrace_process_report.docx");
